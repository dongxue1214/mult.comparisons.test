mult.paired.wilcox=function(data,Sobs,Group,two.sided,BH)
{
  library(stats)
  ...
  for(i in 1:length(unique(merge1$class2)))
  {
    ...
    for(j in  c(unique(data2$class2)) )
    {
      ...
      wilcox=wilcox.test(y ~ x, alternative = c(deparse(substitute(two.sided))),data = sun,paired = TRUE,exact = FALSE)
      p=wilcox$p.value
